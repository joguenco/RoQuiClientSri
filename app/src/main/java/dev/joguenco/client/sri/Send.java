...
public class Send {
    ...
    public static RespuestaSolicitud execute(final String wsdlLocation, final String fileName) throws IOException {
        RespuestaSolicitud response = new RespuestaSolicitud();
        File file = new File(fileName);

        byte[] fileByte = FileLocalUtils.fileToByte(file);
        response = SendWebServices.getResponse(wsdlLocation, file);
